...
class AgendaModel {
  async addCreneauDisponibilite(
    ...
    try {
      ...
      if (differenceMinutes % (dureePratique * 60) !== 0) {
        return {
          success: false,
          message: "La durée doit être un multiple de la durée de la pratique.",
        };
      }
      ...
      let currentHeureFin = new Date(
        currentHeureDebut.getTime() + dureePratique * 3600000
      ); // dureePratique est en minutes
      ...
      while (currentHeureFin <= new Date(`1970-01-01T${heure_fin_dispo}:00`)) {
        ...
        const heureDebutStr = `${currentHeureDebut
          .getHours()
          .toString()
          .padStart(2, "0")}:${currentHeureDebut
          .getMinutes()
          .toString()
          .padStart(2, "0")}`;

        // Récupération des heures et minutes au format HH:MM pour heure_fin_dispo
        const heureFinStr = `${currentHeureFin
          .getHours()
          .toString()
          .padStart(2, "0")}:${currentHeureFin
          .getMinutes()
          .toString()
          .padStart(2, "0")}`;
        ...
        currentHeureFin = new Date(
          currentHeureDebut.getTime() + dureePratique * 3600000
        ); // Ajoute dureePratique minutes
      }
  ...
  async getCreneauxGroupedByDisponibilite(id_prat_det) {
    try {
      ...
      console.error(
        "Erreur dans getCreneauxGroupedByDisponibilite :",
        error.message
      );
      return {
        success: false,
